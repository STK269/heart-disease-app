...
csv_url = st.text_input("Enter CSV URL:", default_url, key="csv_url_input")
...
try:
    ...
    for col in feature_cols:
        ...
        if col == "sex_1":
            choice = st.selectbox("Sex", ["Woman", "Man"], key=f"input_{col}")
            val = 1 if choice == "Man" else 0

        elif col == "fbs_1":
            choice = st.selectbox("Fasting Blood Sugar > 120 mg/dl", ["No", "Yes"], key=f"input_{col}")
            val = 1 if choice == "Yes" else 0

        elif col == "exang_1":
            choice = st.selectbox("Exercise-Induced Angina", ["No", "Yes"], key=f"input_{col}")
            val = 1 if choice == "Yes" else 0

        elif col in ["cp_1", "cp_2", "cp_3"]:
            val = st.number_input(f"{label}", 0, 1, 0, key=f"input_{col}")
    ...
    if st.button("🔮 Predict"):
        ...
        st.success(f"🩺 Prediction (num): **{prediction}** — {prediction_labels.get(prediction, 'Unknown')}")
